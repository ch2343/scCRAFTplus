...
def generate_balanced_dataloader_hardlabel(adata, batch_size, batch_key='batch'):
    ...
    hard_label = adata.obs['hard_label'].cat.codes.values
    ...
    hard_label_tensor = torch.tensor(hard_label[batch_indices], dtype=torch.int64)
    
    # Create a TensorDataset and DataLoader
    combined_dataset = TensorDataset(X_tensor, v_tensor, label_tensor1, label_tensor2, hard_label_tensor)
...
def generate_balanced_dataloader(adata, batch_size, batch_key='batch'):
    ...
    # Extract the feature data
    X_sampled = adata.X[batch_indices, :]
    U_scores_sampled = adata.obsm['U_scores'][batch_indices, :]
